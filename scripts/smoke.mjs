...
const wasm = readFileSync(
	new URL("../pkg/wordle_solver_bg.wasm", import.meta.url),
);
...
assert(
	s.remainingCount() === 2315,
	`initial remainingCount === 2315 (got ${s.remainingCount()})`,
);

// First call returns the optimal opener.
const g1 = s.suggestGuess();
assert(
	typeof g1 === "string" && g1.length === 5,
	`first guess is a 5-letter string (got ${g1})`,
);
...
assert(
	...
	`candidate set shrank (${before} -> ${after})`,
);
console.log(
	"remaining:",
	after,
	"candidates:",
	s.remainingCandidates().slice(0, 3),
);

// Invalid input throws.
let threw = false;
try {
	s.applyFeedback("zzzzz", "BBBBB");
} catch (e) {
	threw = true;
}
